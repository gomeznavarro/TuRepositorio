...
echo  "<h2>Vista de cliente: " . $user->getValueEncoded( "username" ) . " / " . $user->getValueEncoded( "firstName" )."</h2>";
  if ( $errorMessages ) {
...
<div style="margin-left:200px">
    <form action="view_theuser.php" method="post" style="margin-bottom: 50px;" id="registro">
      <div style="width: 30em;">
        ...
        <legend style="font-size:20px;">Datos del cliente</legend>
	...
	if($edit=get_municipios_todo($user->getValueEncoded("RE_pobla")) ){
	  ...
	  $edit = is_array($municipio);
        ...
        <input type="text" disabled="disabled" name="joinDate" id="joinDate" value="<?php echo $user->getValueEncoded( "joinDate" ) ?>" />
        ...
        <?php foreach ( $user->getConocer() as $value => $label ) { ?>
        ...
        <div style="clear: both;">
          ...
          <input type="submit" onclick="return confirm('&iquest;Est&aacute;s seguro?');" name="action" id="deleteButton" value="Delete User" style="margin-right: 20px;" />
        </div>
      </div>
...
?>
    ...
    <div style="width: 30em; margin-top: 20px; text-align: center;">
      <a href="view_users.php?start=<?php echo $start ?>&amp;order=<?php echo $order ?>">Back</a>
    </div>
</div>
<?php
...
function deleteUser() {
  ...
  LogEntryUser::deleteAllForUser( $user->getValue( "id" ) );
  $user->delete();
  displaySuccess();
}
...
?>
    <p>Los cambios se han guardado correctamente <a href="view_users.php?start=<?php echo $start ?>&amp;order=<?php echo $order ?>">Volver a la lista de clientes</a></p>
<?php
  //displayPageFooter();
}
?>