...
class Conexion  // se declara una clase para hacer la conexion con la base de datos
{
	...
	function Conexion()
	{
		...
		*/
		...
		/** quito lo de set name y meto esto/
		 if (!mysql_set_charset('utf8', $conect)) {
...
class Photo
{
    ...
    public static function getPhotos() 
		{
			...
			$obj_photo->executeQuery("select * from photos order by id"); // ejecuta la consulta para traer al photo
	...
	function Photo($nro=0) // declara el constructor, si trae el numero de photo lo busca , si no, trae todos los photos
	{
		...
		{
			$obj_photo=new sQuery();
			$result=$obj_photo->executeQuery("select * from photos where id = $nro"); // ejecuta la consulta para traer al photo 
			$row=mysql_fetch_array($result);
			$this->id=$row['id'];
			$this->title=$row['title']; //titulo
			$this->location=$row['location'];//localiz
			$this->location_mini=$row['location_mini'];//localiz_mini
	...
	function getLocaliz_mini()
	 { return $this->location_mini;}
	function getDescrip()
	...
	function setLocaliz($val)
	 {  $this->location=$val;}
	function setLocaliz_mini($val)
	 {  $this->location_mini=$val;}
	...
	{
			$obj_photo=new sQuery();
			$query="update photos set title='$this->title', location='$this->location', location_mini='$this->location_mini',description='$this->description',publicar='$this->publicar' where id = $this->id";
	...
	{
			$obj_photo=new sQuery();
			$query="insert into photos( title, location, location_mini, description,publicar)values('$this->title', '$this->location', '$this->location_mini','$this->description','$this->publicar')";
	...
	{
			$obj_photo=new sQuery();
			$query="delete from photos where id=$this->id";
