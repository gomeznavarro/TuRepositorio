...
?>
<span> Dado que varios comercios pueden pertenecer a la misma empresa, no se permite aquí la edición de los datos de la empresa a la que pertenece el comercio (aunque sí el cambio de empresa a otra registrada). Para cambiar los datos inherentes a la empresa, h&aacute;galo desde <a class="line" href="view_empresas.php">edici&oacute;n de empresas</a>.</span>

    	<form action="view_member.php" method="post" style="margin-bottom: 50px; margin-top: 0px;" id="registro">
		...
		if($edit=get_shop_todo($member->getValueEncoded( "shopid" )) ){
			...
			foreach ($edit as $shop){  
	  		...
	  		$edit = is_array($shop);
          	// lista de las empresas posibles enviadas por la base de datos
        ...
        <label for="shopid"<?php validateField( "shopid", $missingFields ) ?>>C&oacute;digo comercio *</label>
        ...
        <!-- Para obtener las subcategorías desde clase Member (ahora desde db)
        ...
        <label for="otherInterests">Comentarios</label>
        ...
        <label for="joinDate"<?php validateField( "joinDate", $missingFields ) ?>>Fecha registro *</label>
        ...
        <div style="clear: both;">
          ...
          <input class="envio" type="submit" onclick="return confirm('&iquest;Est&aacute;s seguro?');" name="action" id="deleteButton" value="Borrar Socio" style="margin-right: 20px;" />
        </div>
      </div>
    </form>
                	 	<div class="clear"></div>  
   ...
   <h2>Registro de accesos</h2>
    ...
    <a class="avance" href="../members/view_members.php?start=<?php echo $start ?>&amp;order=<?php echo $order ?>">Ir al listado de socios</a>
    </div>
    <div style="width: 30em; margin-top: 20px; text-align: center;">
...
function deleteMember() {
  ...
  LogEntry::deleteAllForMember( $member->getValue( "shopid" ) );
  $member->delete();
  displaySuccess();
}
...
?>
    ...
    <p><a class="avance" href="../members/view_members.php?start=<?php echo $start ?>&amp;order=<?php echo $order ?>">Ir a la lista de socios</a></p>
    <div style="width: 30em; margin-top: 20px; text-align: center;">
							  <a class="avance" href="javascript:history.go(-1)"><<< Volver</a>
							</div>
   <? } 
